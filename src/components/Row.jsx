...
const Row = ({title, fetchURL, rowId}) => {
  ...
  return (
    <>
      ...
      <h2 className='text-[#fff] font-bold md:text-xl p-4'>
        {title}
      </h2>

      <div className='relative flex items-center group'>
        <MdChevronRight onClick={slideRight}
                        className='bg-[#fff] rounded-full absolute opacity-50 right-0
        ...
        <MdChevronLeft onClick={slideLeft}
                       className='bg-[#fff] rounded-full absolute opacity-50 left-0
                       hover:opacity-100 cursor-pointer z-10 hidden group-hover:block' 
                       size={40}
        />
      </div>
